...
class GetCityByIdImpl @Inject constructor(private val weatherCityDao: WeatherCityDao) :
    GetCityById {
    override suspend fun getCityById(cityId: Int): Result<CurrentWeatherEntityModel> {
        return weatherCityDao.getCityById(cityId)?.let { Result.Success(data = it.toEntity()) }
